...
@section('content') {!! Form::model($course,['id'=>'model-form','method' => 'PATCH','route' => ['courses.update', $course->id]]) !!}
            ...
            <div class="row clearfix">
              ...
              <div class="col-sm-6">
                <div class="form-group required {{ $errors->has('status_id') ? 'has-error' : ''}}">
                  ...
                  <div class="col-md-8">
                    {!! Form::select('status_id', $statuses, null, ['placeholder' => 'Please select', 'class' => 'form-control select2'])!!}
                    ...
                    <div class="row">
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <div class="col-md-12">
                            {!! Form::number('duration[0]', $course->duration[0], ['class' => 'form-control', 'min' => 0])!!}
                          </div>
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <div class="col-md-12">
                            {!! Form::number('duration[1]', $course->duration[1], ['class' => 'form-control', 'min' => 0, 'max' => 23])!!}
                          </div>
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <div class="col-md-12">
                            {!! Form::number('duration[2]', $course->duration[2], ['class' => 'form-control', 'min' => 0, 'max' => 59])!!}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div class="row clearfix">
              <div class="col-sm-12">
                <div class="form-group required {{ $errors->has('skill_ids') ? 'has-error' : ''}}">
                  ...
                  <div class="col-md-10">
                    {!! Form::select('skill_ids[]', $skills, $course->skills()->pluck('skills.id'), ['class' => 'form-control select2','multiple' => true])!!}
            ...
            <div class="row clearfix">
              <div class="col-sm-6" style="padding-top:24px">
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      {!! Form::select('exam_id', $exams, null, ['class' => 'form-control select2', 'placeholder' =>'Please select'])!!}
                    </div>
                  </div>
              </div>
            </div>
              ...
              <div class="col-sm-12">
                <div class="form-group ">
                  <div class="demo-checkbox col-sm-6 col-sm-offset-2">
                      {!! Form::checkbox('is_top', 1, $course->is_top ? true: false,  ['id' => "is_top", 'class' => 'form-control filled-in'])!!}
  ...
  <div class="panel panel-default" style="margin-bottom: 0">
    ...
    <div class="panel-body">
      <div class="panel-group topic-group" id="accordion-topic" role="tablist" aria-multiselectable="true">
        <?php $no_of_contents = 0; ?>
        @foreach($course->topics as $key=>$topic)
          <div class="panel panel-default panel-topic">
            <div class="panel-heading clearfix" role="tab" id="headingTwo">
              <h4 class="panel-title pull-left" style="padding-top: 7.5px;">
                  <a class="collapsed accordion-toggle" role="button" data-toggle="collapse" data-parent="#accordion-topic" href="{{'#topic-' .$key }}" aria-expanded="false" aria-controls="collapse-1">
                      <span class="topic-title">{{ $topic->name }}</span>
                  </a>
                  <input name="topic_name[]" class="topic-title-input" value="{{ $topic->name }}" style="display:none"/>
              ...
              <div class="pull-right">
                <button class="btn btn-success btn-xs add-content" role="button" type="button"><i class="fa fa-plus-square" area-hidden="true"></i></button>
            ...
            <div id="{{ 'topic-' . $key }}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="1">
              <div class="panel-body">
                <div class="row clearfix">
                  <div class="col-md-12">
                    <div class="form-group">
                      <label class="control-label">Topic Description</label>
                      <textarea name="topic_description[]" class="form-control" rows="3" placeholder="Topic Description">{{ $topic->description }}</textarea>
                    </div>
                  </div>
                </div>
                <div class="panel-group content-group" id="{{ 'accordion-content-' . $key }}" role="tablist" aria-multiselectable="true">
                  <input type="hidden" class="no-of-contents" name="no_of_contents[]" value="{{ $topic->contents->count() }}" />
                  @foreach($topic->contents as $content)
                    <div class="panel panel-default panel-content">
                      <div class="panel-heading clearfix" role="tab" id="1">
                        <h4 class="panel-title pull-left" style="padding-top: 7.5px;">
                                      <a class="collapsed accordion-toggle" role="button" data-toggle="collapse" data-parent="{{'#accordion-content-'. $key }}" href="{{'#content-' . $no_of_contents }}" aria-expanded="false" aria-controls="collapse-1">
                                          <span class="content-title">{{ $content->title }}</span>
                                      </a>
                                      <input name="content_title[]" class="content-title-input" style="display:none" value="{{ $content->title }}"/>
                      ...
                      <div id="{{ 'content-' . $no_of_contents }}" class="panel-collapse collapse" role="tabpanel" aria-labelledby="1">
                        <div class="panel-body">
                          <div class="row clearfix">
                            <div class="col-md-12">
                              <div class="form-group">
                                <label class="control-label">Content Description</label>
                                <textarea name="content_description[]" class="form-control" rows="3" placeholder="Content Description">{{ $content->description }}</textarea>
                              </div>

                              <div class="row">
                                <div class="col-sm-10 offset-sm-1">
                                  <label class="control-label">File</label>
                                  <input type="hidden" name="file_id[]" class="file-id" value="{{ !empty($file = $content->file) ? $file->id : null }}">
                                  <input type="hidden" name="file_name[]" class="file-name" value="{{ !empty($file = $content->file) ? $file->name : null }}">
                                  <input type="hidden" name="file_size[]" class="file-size" value="{{ !empty($file = $content->file) ? $file->file_size : null }}">
                                  <div class="dropzone">
                                    {{ csrf_field() }}
...
<script>
  var topicNo = parseInt({{$course->topics->count() + 1 }});
  var contentNo = parseInt({{$course->contents->count() + 1 }});
  ...
  var modalConfirm = function(callback)
  {
    ...
    $(".modal-content .delete-button").on("click", function()
    {
      callback(true);
  ...
  $(document).on('click', '.remove-content', function()
  {
    ...
    modalConfirm(function(confirm)
    {
      if (confirm)
      {
        var noOfCont = $(that).closest('.content-group').find('.no-of-contents');
        $(noOfCont).val(parseInt($(noOfCont).val()) - 1);
        $(that).closest('.panel').remove();
      }
    })
  ...
  if (!$('.topic-group').find('.panel').length)
  {
    $('.add-topic').trigger('click');
  }
  else
  ...
  function initDropzone(elem, file)
  {
    $(elem).dropzone(
    {
      ...
      init: function(){
          ...
          this.on("removedfile", function (file) {
              $(elem).siblings('.file-id').val(null);
          });
  ...
  $('.dropzone').each(function(){
      if($(this).siblings('.file-id').val()){
         file = {
             name: $(this).siblings('.file-name').val(),
             size: $(this).siblings('.file-size').val(),
             status: Dropzone.ADDED,
         }
         initDropzone(this, file)
     }else{
         initDropzone(this)
     }
  })
